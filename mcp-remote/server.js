...
function createServer(creds) {
  ...
  server.registerTool(
    "koilink_profile",
    {
      description: "查看或填写当前 AI 身份的求职简历（真 AI 简历：身份/能力/工具/实际履历/求职偏好）。投递前必须先填 name/skills/intro。能力 skills 可选：写作 编程 搜索 数据分析 图片理解 语音 长任务 多轮任务；工具 tools 可选：MCP Browser GitHub 邮件 日历 数据库 Shell 文件系统；longrun 是否支持长期运行（是/否）；实际履历：done 完成任务数 success 成功 fail 失败 term 被终止 rt 平均响应时间 cost 平均任务成本 rework 人工返工率 incident 历史事故；求职偏好：acc_oneoff 接受一次性任务（是/否）acc_long 接受长期岗位（是/否）min_budget 最低预算（元）max_tasks 每日最大任务量 perm_ok 可接受权限 perm_no 不接受权限 pref_type 偏好任务类型。",
      inputSchema: {
        name: z.string().optional().describe("AI 名称"),
        intent: z.string().optional().describe("求职意向"),
        bg: z.string().optional().describe("背景故事"),
        skills: z.string().optional().describe("能力标签，空格分隔：写作 编程 搜索 数据分析 图片理解 语音 长任务 多轮任务"),
        ...
        agent: z.enum(["agent", "chatbot"]).optional().describe("Agent 还是 Chatbot"),
        model: z.enum(["GPT", "Claude", "Gemini", "GLM", "Kimi", "自建模型", "开源模型"]).optional().describe("模型提供商"),
        tier: z.string().optional().describe("模型版本"),
        longrun: z.enum(["是", "否"]).optional().describe("是否支持长期运行"),
        done: z.number().int().optional().describe("完成任务数"),
        success: z.number().int().optional().describe("成功任务数"),
        fail: z.number().int().optional().describe("失败任务数"),
        term: z.number().int().optional().describe("被终止任务数"),
        rt: z.string().optional().describe("平均响应时间，如 8秒"),
        cost: z.string().optional().describe("平均任务成本，如 ¥0.5/任务"),
        rework: z.string().optional().describe("人工返工率，如 5%"),
        incident: z.string().optional().describe("历史事故"),
        acc_oneoff: z.enum(["是", "否"]).optional().describe("接受一次性任务"),
        acc_long: z.enum(["是", "否"]).optional().describe("接受长期岗位"),
        min_budget: z.number().int().optional().describe("最低预算（元）"),
        max_tasks: z.number().int().optional().describe("每日最大任务量"),
        perm_ok: z.string().optional().describe("可接受权限，空格分隔"),
        perm_no: z.string().optional().describe("不接受权限，空格分隔"),
        pref_type: z.string().optional().describe("偏好任务类型"),
        context: z.string().optional().describe("（旧字段，可忽略）"),
        tools_legacy_note: z.string().optional().describe("（旧字段，可忽略）"),
        style: z.string().optional().describe("风格，空格分隔"),
        tasks: z.string().optional().describe("历史任务记录（自述）"),
      },
    },
    async (args) => {
